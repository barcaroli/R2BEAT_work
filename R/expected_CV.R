...
#' 
#' @return a dataframe containing the expected coefficients of variation in each domain for each target variable
#' 
#' @examples
...
{
  nvars <- ncol(errors) - 1
  colnames(errors)[2:(nvars+1)] <- paste("cv(Y", c(1:nvars), ")", sep = "")
  eval(parse(text=paste0("if (is.null(strata$",alloc_variable,")) stop('There is no allocation of units in strata')")))
  ...
  for (d in c(1:ndom)) {
    ...
    for (i in (1:nrow(strata2))) {
      ...
      strata3 <- strata2[i, ]
      for (j in 1:nvars) {
        eval(parse(text =paste0("n_h <- strata3$",alloc_variable)))
        N_h <- strata3$N
        eval(parse(text=paste0("S_h <- strata3$S",j)))
        eval(parse(text = paste0("M_h <- strata3$M", j)))
        Y_h <- N_h * M_h
